...
import { 
  ...
  Heart, Bookmark, Loader2, X, Share2 
} from 'lucide-react';
...
export default function WisataDetail({ wisataId, onBack }) {
  ...
  const handleShare = async () => {
    const shareData = {
        title: data.name,
        text: `Ayo liburan ke ${data.name} di Cilacap! Lokasi: ${data.location}.`,
        url: window.location.href // Mengambil URL aplikasi saat ini
    };

    try {
        if (navigator.share) {
            // Jika di HP (Support Native Share)
            await navigator.share(shareData);
        } else {
            // Jika di Laptop (Fallback Copy)
            await navigator.clipboard.writeText(`${shareData.text} Cek di aplikasi Pariwisata Cilacap!`);
            alert('Info wisata berhasil disalin ke clipboard!');
        }
    } catch (err) {
  ...
  useEffect(() => {
    if (wisataId) { fetchDetail(); checkInteractions(); }
  }, [wisataId]);
      ...
      <div className="max-w-4xl mx-auto px-5 py-8 -mt-6 relative z-10 bg-white rounded-t-3xl md:rounded-none">
         <div className="flex flex-col md:flex-row gap-8 md:gap-12">
            <div className="flex-1 space-y-8">
                <div><h3 className="text-lg font-bold text-slate-800 mb-3 border-l-4 border-emerald-500 pl-3">Tentang Destinasi</h3><p className="text-slate-600 leading-relaxed text-sm text-justify">{data.description || "Deskripsi belum tersedia."}</p></div>
                <div><h3 className="text-lg font-bold text-slate-800 mb-4 border-l-4 border-emerald-500 pl-3">Fasilitas Tersedia</h3><div className="grid grid-cols-2 gap-3">{(data.facilities || []).map((fas, idx) => (<div key={idx} className="flex items-center p-3 bg-slate-50 rounded-xl border border-slate-100"><CheckCircle className="w-4 h-4 text-emerald-500 mr-3 flex-shrink-0" /><span className="text-sm text-slate-600 font-medium">{fas}</span></div>))}</div></div>
            ...
            <div className="md:w-80 flex-shrink-0">
                <div className="bg-white border border-slate-100 shadow-xl rounded-3xl p-6 sticky top-24">
                    ...
                    <div className="flex justify-between mb-4 pb-4 border-b border-slate-50"><div className="flex items-center text-slate-500"><Tag className="w-4 h-4 mr-2" /><span className="text-sm">Harga Tiket</span></div><span className="text-xl font-bold text-emerald-600">{data.price}</span></div>
                    <div className="flex justify-between mb-6"><div className="flex items-center text-slate-500"><Clock className="w-4 h-4 mr-2" /><span className="text-sm">Jam Buka</span></div><span className="text-sm font-semibold text-slate-800">08.00 - 17.00 WIB</span></div>
                    <button onClick={handleOpenMap} className="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl hover:bg-emerald-600 transition-colors shadow-lg flex items-center justify-center gap-2 mb-3"><MapPin className="w-4 h-4" /> Petunjuk Arah</button>
